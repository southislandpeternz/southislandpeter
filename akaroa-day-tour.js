/**
 * Loads gallery/akaroa-day-tour.json and injects Photo Library images only.
 * Usage: data-akaroa-gallery="hero|cover|thumbs|section" + data-akaroa-category slug/id
 */
(function () {
  "use strict";

  const MANIFEST_URL = "gallery/akaroa-day-tour.json";
  ...
  function loadManifest() {
    if (!manifestPromise) {
      manifestPromise = fetch(resolveManifestUrl())
        ...
        .catch((err) => {
          console.warn("[akaroa-day-tour]", err);
          return null;
        });
    }
  ...
  function findCategory(manifest, el) {
    const slug = el.getAttribute("data-akaroa-category");
    const id = el.getAttribute("data-akaroa-category-id");
  ...
  function fillHero(manifest, el) {
    const hero = manifest.heroPrimary || manifest.categories?.find((c) => c.id === "01-Hero")?.images?.[0];
    if (!hero) return;
  ...
  function fillCardThumbs(manifest, el) {
    const picks = (el.getAttribute("data-akaroa-picks") || "02-Alpaca-Farm,03-Lobster-Cruise,04-Wildlife")
      .split(",")
      .map((s) => s.trim());
  ...
  function fillSlot(manifest, el) {
    const catId = el.getAttribute("data-akaroa-category-id") || el.getAttribute("data-akaroa-category");
    const index = Number(el.getAttribute("data-akaroa-index") || "0");
  ...
  function fillThumbs(manifest, el) {
    ...
    const images = cat?.images?.length
      ? cat.images
      : manifest.categories?.find((c) => c.id === "03-Lobster-Cruise")?.images || [];
    const figures = el.querySelectorAll("figure");
    figures.forEach((figure, i) => {
  ...
  function fillSection(manifest, el) {
    ...
    const grid = el.querySelector("[data-akaroa-grid]") || el;
  ...
  function fillMeta(manifest) {
    const hero = manifest.heroPrimary;
    if (!hero) return;
    const titleEl = document.querySelector("[data-akaroa-meta='title']");
    const metaDesc = document.querySelector("meta[name='description'][data-akaroa-meta='description']");
  ...
  async function init() {
    const nodes = document.querySelectorAll("[data-akaroa-gallery]");
    const slots = document.querySelectorAll("[data-akaroa-slot]");
    ...
    nodes.forEach((el) => {
      const role = el.getAttribute("data-akaroa-gallery");
      switch (role) {
        case "hero":
    ...
    document.dispatchEvent(new CustomEvent("akaroa-gallery-ready", { detail: manifest }));
  }
